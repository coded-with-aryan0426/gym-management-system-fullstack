...
\usepackage{xcolor}
\usepackage[a4paper,left=1.5in,right=1.0in,top=1.0in,bottom=1.0in,headheight=15pt,headsep=0.3in,footskip=0.4in]{geometry}
...
\fancyfoot[L]{\fontsize{10}{12}\selectfont BTech IT \enspace School of Computer Science Engineering \& Technology}
...
\titleformat{\chapter}[display]
  {\normalfont\Large\bfseries}{\chaptername\ \thechapter}{10pt}{\Large\bfseries}
...
\textbf{UNIVERSITY:} ITM SLS Baroda University\textbackslash{}
\textbf{SCHOOL / FACULTY:} School of Computer Science Engineering \&
Technology\textbackslash{} \textbf{PROGRAMME:} B.Tech (IT)

\vspace{0.2in}

\{\Large \textbf{Report Title}\}\textbackslash{} \vspace{0.1in}
...
\{\Large \textbf{Project / Dissertation Status Declaration}\}\textbackslash{}
\vspace{0.1in} I hereby declare and authorise the following:
...
\textbf{Student Name:} Suthar Aryan Sujalkumar\textbackslash{}
\textbf{Enrolment Number:} 23C25512\textbackslash{} \textbf{Programme:}
B.Tech (Information Technology)\textbackslash{} \textbf{Semester:}
{[}FILL\_IN: e.g.~Semester 8{]}\textbackslash{} \textbf{Internship
Period:} 17th November 2025 -- 17th May 2026\textbackslash{}
\textbf{Organisation / Company:} Bharti Soft Tech Pvt. Ltd.

\vspace{0.4in}

\textbf{Student Signature:} \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
\hfill **Date:** \_\_\_\_\_\_\_\_\_\_\_\_\_\_

\vspace{0.2in}

\textbf{Supervisor / Lecturer:} Dr.~Ashutosh Abhangi
...
\textbf{Student Name:} Suthar Aryan Sujalkumar

\textbf{Enrolment Number:} 23C25512

\textbf{Programme:} B.Tech (Information Technology)

\textbf{Date:} \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

\textbf{Signature:} \_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_
...
unwavering encouragement and moral support.
...
\textbf{Keywords:} Gym Management System, React, Spring Boot, Oracle,
Role-Based Access Control, JWT, WebSocket.
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Custom Web Application Development} & Full-stack web
applications built to client specifications \\
\textbf{Mobile Application Development} & Cross-platform mobile apps for
Android and iOS \\
\textbf{API Development \& Integration} & RESTful API design and
third-party service integration \\
\textbf{Database Design \& Optimisation} & Schema design, query tuning,
and migration services \\
\textbf{IT Consultancy} & Technical advisory for digital transformation
projects \\
\end{longtable}
}
...
Friday & Code reviews, pull request merges, sprint review, and
retrospective \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
Multi-role authentication and authorisation & Achieved --- JWT and RBAC
implemented across all four roles \\
Membership lifecycle management & Achieved --- complete purchase,
approval, expiry, and renewal workflows \\
Personal training booking system & Achieved --- session creation,
cancellation, and rating workflows functional \\
Real-time chat between roles & Achieved --- WebSocket STOMP messaging
with delivery status \\
Financial reporting and analytics & Achieved --- revenue charts,
transaction tables, and KPI dashboard \\
Security compliance with OWASP guidelines & Achieved --- BCrypt, input
validation, CORS, and JWT verified \\
Sub-200ms API response times & Achieved --- validated under standard
load conditions \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Mobile Application} & High & Native iOS/Android companion app
for members \\
\textbf{Payment Gateway Integration} & High & Stripe or Razorpay for
online membership payments \\
\textbf{AI-Powered Workout Recommendations} & Medium & Machine learning
model for personalised plans \\
\textbf{Microservices Migration} & Medium & Decompose monolith into
independently scalable services \\
\textbf{Multi-language Support (i18n)} & Medium & Internationalisation
for non-English markets \\
\textbf{GDPR Compliance Module} & High & Data export, anonymisation, and
right-to-erasure features \\
\textbf{Video Conferencing Integration} & Low & Embedded video for
remote PT sessions \\
\textbf{Advanced Analytics Dashboard} & Medium & Predictive churn
analysis, retention metrics \\
\end{longtable}
}
...
\CommentTok{\# Configure database connection in application.properties}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{VITE\_API\_BASE\_URL} & Backend API URL &
\texttt{http://localhost:8080/api} \\
\texttt{VITE\_WS\_URL} & WebSocket server URL &
\texttt{ws://localhost:8080/ws} \\
\texttt{spring.datasource.url} & JDBC connection string &
\texttt{jdbc:oracle:thin:@...} \\
\texttt{jwt.secret} & JWT signing secret (min 256-bit) &
\texttt{{[}secure-random-string{]}} \\
\texttt{spring.mail.host} & SMTP host for email OTP &
\texttt{smtp.gmail.com} \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{users} & \texttt{user\_id} & All user accounts across roles \\
\texttt{roles} & \texttt{role\_id} & Role definitions (MEMBER, TRAINER,
OWNER, ADMIN) \\
\texttt{gyms} & \texttt{gym\_id} & Gym tenant records \\
\texttt{memberships} & \texttt{membership\_id} & Active and historical
memberships \\
\texttt{membership\_packages} & \texttt{package\_id} & Configured
membership packages \\
\texttt{pt\_sessions} & \texttt{session\_id} & Personal training session
bookings \\
\texttt{trainer\_details} & \texttt{trainer\_id} & Trainer profile
extensions \\
\texttt{conversations} & \texttt{conversation\_id} & Chat conversation
threads \\
\texttt{messages} & \texttt{message\_id} & Individual chat messages \\
\texttt{notifications} & \texttt{notification\_id} & System
notifications \\
\end{longtable}
}
...
POST & \texttt{/api/auth/login} & Authenticate and receive JWT \\
POST & \texttt{/api/auth/refresh} & Refresh expired JWT \\
POST & \texttt{/api/auth/logout} & Invalidate session \\
...
\bottomrule\noalign{}
\endlastfoot
GET & \texttt{/api/memberships/packages} & List available packages \\
POST & \texttt{/api/memberships/request} & Submit membership purchase
request \\
PUT & \texttt{/api/memberships/\{id\}/approve} & Owner approves
membership \\
PUT & \texttt{/api/memberships/\{id\}/reject} & Owner rejects
membership \\
...
\bottomrule\noalign{}
\endlastfoot
GET & \texttt{/api/sessions/trainers} & Browse available trainers \\
POST & \texttt{/api/sessions/book} & Book a PT session \\
PUT & \texttt{/api/sessions/\{id\}/complete} & Mark session as
complete \\
PUT & \texttt{/api/sessions/\{id\}/cancel} & Cancel a booked session \\
