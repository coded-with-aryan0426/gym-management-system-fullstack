...
\usepackage{xcolor}
\usepackage[a4paper,left=0.70in,right=0.70in,top=1.0in,bottom=1.0in,headheight=14pt,headsep=0.15in,footskip=0.4in]{geometry}
...
\titleformat{\chapter}[hang]
  {\normalfont\Large\bfseries}{\chaptername\ \thechapter:}{6pt}{\Large\bfseries}
...
\includegraphics[width=1.6in]{styles/university_logo.png}
...
\chapter*{Report Status Declaration
Form}\label{report-status-declaration-form}
...
% --- List of Tables and List of Figures on the same page ---
% Patch \clearpage to a no-op so \listoftables/\listoffigures don't force new pages
\begingroup
...
\noindent
\footnotesize
\begin{tabular}{@{} p{1.2cm} p{\dimexpr0.47\linewidth-1.2cm-\tabcolsep} @{\hspace{0.06\linewidth}} p{1.2cm} p{\dimexpr0.47\linewidth-1.2cm-\tabcolsep} @{}}
\toprule
...
\chapter{Literature Review - Gym Management
System}\label{literature-review---gym-management-system}

\section{Introduction}\label{introduction-1}

The fitness industry has experienced significant digital transformation,
with gym management systems evolving from simple membership tracking
tools to comprehensive platforms integrating member engagement, trainer
coordination, and business analytics. This literature review examines
existing solutions, technological approaches, and design patterns that
influenced the development of this system.

\medskip\noindent

\rule{\textwidth}{0.4pt}\medskip

\section{Existing Gym Management
Systems}\label{existing-gym-management-systems}

\subsection{Commercial Solutions}\label{commercial-solutions}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Mindbody} & Booking, payments, marketing & High cost, complex
setup \\
\textbf{Zen Planner} & Membership, billing, reporting & Limited
customization \\
\textbf{GymMaster} & Access control, POS, member app &
Desktop-focused \\
\textbf{PushPress} & CRM, automation, mobile app & Pricing tier
restrictions \\
\textbf{Wodify} & CrossFit focus, performance tracking & Niche market
focus \\
\end{longtable}
}

\subsection{Gap Analysis}\label{gap-analysis}

Existing solutions often present challenges: - \textbf{Cost Barriers}:
Enterprise pricing models exclude small gyms - \textbf{Inflexibility}:
Limited customization for unique workflows - \textbf{Integration
Complexity}: Difficulty integrating with existing tools -
\textbf{Feature Overload}: Unnecessary complexity for basic needs

Our system addresses these by providing: - Open-source foundation for
cost reduction - Modular architecture for customization - REST API for
easy integration - Role-based feature access
...
\subsection{Frontend: React with
TypeScript}\label{frontend-react-with-typescript}

\textbf{Selection Rationale:} - \textbf{Component-Based Architecture}:
Enables reusable UI components {[}1{]} - \textbf{Virtual DOM}: Optimizes
rendering performance {[}2{]} - \textbf{TypeScript}: Adds type safety,
reducing runtime errors by 15-25\% {[}3{]} - \textbf{Ecosystem}: Rich
library support (React Router, Axios, Framer Motion)

\textbf{Alternatives Considered:} - Vue.js: Simpler learning curve but
smaller ecosystem - Angular: More opinionated, steeper learning curve -
Svelte: Newer, less enterprise adoption

\subsection{Backend: Spring Boot with
Java}\label{backend-spring-boot-with-java}

\textbf{Selection Rationale:} - \textbf{Enterprise Ready}: Battle-tested
in production environments {[}4{]} - \textbf{Dependency Injection}:
Promotes loose coupling and testability - \textbf{Spring Security}:
Comprehensive authentication/authorization {[}5{]} -
\textbf{JPA/Hibernate}: Robust ORM with Oracle support -
\textbf{Convention over Configuration}: Rapid development

\textbf{Alternatives Considered:} - Node.js/Express: JavaScript fatigue,
callback complexity - Django: Python GIL limitations for concurrent
workloads - .NET Core: Smaller open-source community

\subsection{Database: Oracle}\label{database-oracle}

\textbf{Selection Rationale:} - \textbf{ACID Compliance}: Data integrity
for financial transactions - \textbf{Scalability}: Handles growing
member data - \textbf{Enterprise Features}: Advanced security, auditing
- \textbf{JPA Compatibility}: Seamless Hibernate integration
...
\subsection{Multi-Tenancy}\label{multi-tenancy}

The system implements \textbf{Schema-Level Multi-Tenancy} where: - Each
gym operates as an isolated tenant - Data segregation ensures privacy -
Shared infrastructure reduces costs

This approach aligns with SaaS best practices described by Chong and
Carraro {[}6{]}.

\subsection{Role-Based Access Control
(RBAC)}\label{role-based-access-control-rbac}

Implementation follows NIST RBAC model {[}7{]}: - \textbf{Core RBAC}:
User-role and role-permission assignments - \textbf{Hierarchical RBAC}:
Role inheritance (Admin \textgreater{} Owner \textgreater{} Trainer
\textgreater{} Member) - \textbf{Constrained RBAC}: Separation of duties
for sensitive operations

\subsection{JWT-Based Authentication}\label{jwt-based-authentication}

Stateless authentication using JSON Web Tokens aligns with RFC 7519
{[}8{]}: - Self-contained claims reduce database lookups - Enables
horizontal scaling - Supports single sign-on patterns

\medskip\noindent

\rule{\textwidth}{0.4pt}\medskip

\section{Real-Time Communication}\label{real-time-communication}

\subsection{WebSocket Implementation}\label{websocket-implementation}

The chat system utilizes WebSocket (RFC 6455) {[}9{]} for: -
Bi-directional communication - Lower latency than polling (\textless{}
50ms vs 1000ms+) - Reduced server load

Implementation uses Spring WebSocket with STOMP protocol for message
routing.

\subsection{Push Notifications}\label{push-notifications}

Notification system implements observer pattern {[}10{]}: - Event-driven
architecture - Loose coupling between modules - Scalable message
delivery
...
\tightlist
\item
  \textbf{Password Hashing}: BCrypt with 12 rounds (recommended by OWASP
  {[}11{]})
\item
  \textbf{Token Security}: JWT with HMAC-SHA512 signature
\item
  \textbf{Session Management}: Stateless tokens with expiry
\end{itemize}
...
\tightlist
\item
  \textbf{Input Validation}: Server-side validation prevents injection
\item
  \textbf{CORS Policy}: Whitelist-based origin control
\item
  \textbf{Soft Deletes}: Data recovery support
\end{itemize}
...
\subsection{Industry Standards}\label{industry-standards}

\begin{itemize}
\tightlist
\item
  \textbf{GDPR Compliance}: Data protection for EU markets
\item
  \textbf{PCI-DSS}: Payment data security guidelines
\item
  \textbf{WCAG 2.1}: Accessibility standards
\end{itemize}
...
The following table presents a qualitative feature comparison between
the Smart Gym Management System and leading commercial competitors.
Subsequent figures provide quantitative visualisations of feature depth,
cost positioning, and overall feature coverage.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth,keepaspectratio]{styles/chart_feature_comparison.png}
\caption{Feature Depth Comparison -- Smart GMS vs Market Competitors (Score out of 10)}
\end{figure}

As illustrated in the figure above, the Smart Gym Management System
achieves the maximum feature depth score (10/10) across all eight
evaluated dimensions. Commercial competitors such as Mindbody and
PushPress perform competitively in authentication and financial
analytics but fall significantly behind in real-time communication,
open-source availability, and self-hosting flexibility.

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth,keepaspectratio]{styles/chart_cost_comparison.png}
\caption{Monthly Subscription Cost Comparison -- Smart GMS vs Commercial Platforms (USD)}
\end{figure}

The cost comparison reveals a significant economic advantage of the
Smart Gym Management System. Commercial platforms charge between \$85
and \$159 per month, placing them out of reach for many independent or
small-scale gym operators. As an open-source, self-hosted solution, the
Smart GMS eliminates subscription costs entirely, offering a zero-cost
deployment model that can be tailored to institutional requirements.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth,keepaspectratio]{styles/chart_feature_pie.png}
\caption{Overall Feature Coverage Distribution Across Gym Management Platforms (\%)}
\end{figure}

The pie chart demonstrates that the Smart Gym Management System accounts
for the most comprehensive feature coverage among the evaluated
platforms. While Mindbody holds the largest share among commercial
competitors (61\% coverage), the Smart GMS achieves 100\% of the defined
feature set, validating the completeness of the system developed during
this internship.
...
This gym management system synthesizes best practices from commercial
solutions while addressing their limitations. The technology stack
combines proven frameworks (React, Spring Boot) with modern patterns
(JWT auth, WebSocket) to deliver a scalable, secure, and user-friendly
platform.

Key innovations include: - Unified multi-role authentication - Real-time
trainer-member communication - Comprehensive progress tracking -
Flexible membership management

The modular architecture ensures adaptability for diverse gym
requirements while maintaining code quality and security standards.
...
\chapter{System Architecture - Gym Management
System}\label{system-architecture---gym-management-system}

\section{High-Level Architecture}\label{high-level-architecture}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_system_architecture_0.png}
\end{figure}
...
\textbf{Spring Security} & 6.x & Authentication \& Authorization \\
\textbf{Spring Data JPA} & 3.x & Data Access \\
\textbf{Spring WebSocket} & - & Real-time Messaging \\
\textbf{Hibernate} & 6.x & ORM \\
\textbf{Lombok} & 1.18.x & Boilerplate Reduction \\
\textbf{JJWT} & 0.11.x & JWT Handling \\
\end{longtable}
}

\subsection{Database \& Infrastructure}\label{database-infrastructure}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Google OAuth} & Social Login \\
\textbf{Facebook OAuth} & Social Login \\
\textbf{SMTP} & Email Notifications \\
\textbf{Twilio} & SMS OTP \\
\end{longtable}
}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_system_architecture_1.png}
\end{figure}
...
\section{Authentication Flow}\label{authentication-flow}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_system_architecture_2.png}
\end{figure}
...
\section{Database Architecture}\label{database-architecture}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_system_architecture_3.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_system_architecture_4.png}
\end{figure}
...
\textbf{Authorization} & RBAC & Custom Permission Service \\
\textbf{Password} & BCrypt & Spring Security \\
\textbf{Session} & Stateless & JWT Tokens \\
\textbf{2FA} & OTP & Email/SMS Verification \\
\textbf{Data} & Soft Deletes & Entity Annotations \\
\textbf{CORS} & Whitelist & Spring CORS Config \\
\end{longtable}
}
...
\tightlist
\item
  \textbf{Multi-Tenancy}: Each gym is a separate tenant with isolated
  data
\item
  \textbf{Caching}: Spring Cache for frequently accessed data
\item
  \textbf{Connection Pooling}: HikariCP for database connections
\item
  \textbf{Lazy Loading}: JPA relationship optimization
\item
  \textbf{Stateless Auth}: JWT enables horizontal scaling
\item
  \textbf{WebSocket}: Dedicated connection management for real-time
  features
\end{enumerate}

\clearpage

\chapter{Entity-Relationship Diagram - Gym Management
System}\label{entity-relationship-diagram---gym-management-system}

\section{User \& Authentication
Entities}\label{user-authentication-entities}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_er_diagram_0.png}
\end{figure}
...
\section{Gym Management Entities}\label{gym-management-entities}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_er_diagram_1.png}
\end{figure}
...
\section{Membership Entities}\label{membership-entities}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_er_diagram_2.png}
\end{figure}
...
\section{Training \& Session Entities}\label{training-session-entities}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_er_diagram_3.png}
\end{figure}
...
\section{Communication Entities}\label{communication-entities}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_er_diagram_4.png}
\end{figure}
...
\section{Notification Entities}\label{notification-entities}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_er_diagram_5.png}
\end{figure}
...
\section{Analytics Entities}\label{analytics-entities}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_er_diagram_6.png}
\end{figure}
...
\section{Entity Summary Table}\label{entity-summary-table}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Auth} & Users, Roles, Permissions & Many-to-many via mapping
tables \\
\textbf{Gym} & Gyms, Settings, Staff, Equipment & Owner-owned,
one-to-many \\
\textbf{Members} & Packages, Memberships, Transactions & Package →
Membership → Transaction \\
\textbf{Training} & Assignments, Sessions, Ratings, Progress &
Trainer-Member assignments \\
\textbf{Chat} & Conversations, Messages, Attachments & Conversation
hierarchy \\
\textbf{Notify} & Notifications, Settings & User notifications \\
\textbf{Analytics} & Gym Analytics, Trainer Stats & Period-based
aggregations \\
\end{longtable}
}
...
\chapter{UML Diagrams - Gym Management
System}\label{uml-diagrams---gym-management-system}

\section{Class Diagram - Core User \&
Auth}\label{class-diagram---core-user-auth}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_0.png}
\end{figure}
...
\section{Class Diagram - Gym \&
Membership}\label{class-diagram---gym-membership}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_1.png}
\end{figure}
...
\section{Class Diagram - Training \&
Sessions}\label{class-diagram---training-sessions}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_2.png}
\end{figure}
...
Communication}\label{class-diagram---communication}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_3.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_4.png}
\end{figure}
...
\section{Sequence Diagrams}\label{sequence-diagrams}

\subsection{User Authentication Flow}\label{user-authentication-flow}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_5.png}
\end{figure}

\subsection{Membership Purchase Flow}\label{membership-purchase-flow}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_6.png}
\end{figure}

\subsection{PT Session Booking Flow}\label{pt-session-booking-flow}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_7.png}
\end{figure}
...
\subsection{User Registration Process}\label{user-registration-process}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_8.png}
\end{figure}

\subsection{Membership Approval
Workflow}\label{membership-approval-workflow}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_9.png}
\end{figure}

\subsection{Chat Message Flow}\label{chat-message-flow}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_uml_diagrams_10.png}
\end{figure}

\clearpage

\chapter{Data Flow Diagrams - Gym Management
System}\label{data-flow-diagrams---gym-management-system}

\section{Level 0: Context Diagram}\label{level-0-context-diagram}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_dfd_diagrams_0.png}
\end{figure}
...
\section{Level 1: Main Processes}\label{level-1-main-processes}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_dfd_diagrams_1.png}
\end{figure}
...
\section{Level 2: Detailed
Sub-Processes}\label{level-2-detailed-sub-processes}

\subsection{Authentication (Process
1.0)}\label{authentication-process-1.0}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_dfd_diagrams_2.png}
\end{figure}

\subsection{Membership (Process 2.0)}\label{membership-process-2.0}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_dfd_diagrams_3.png}
\end{figure}

\subsection{Training (Process 3.0)}\label{training-process-3.0}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_dfd_diagrams_4.png}
\end{figure}

\subsection{Communication (Process
4.0)}\label{communication-process-4.0}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_dfd_diagrams_5.png}
\end{figure}

\subsection{Analytics (Process 5.0)}\label{analytics-process-5.0}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_dfd_diagrams_6.png}
\end{figure}
...
\bottomrule\noalign{}
\endlastfoot
Users & System users & user\_id, email, roles \\
Memberships & Subscriptions & id, user\_id, status \\
Sessions & PT records & id, trainer\_id, date \\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_0.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_1.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_2.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_3.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_4.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_5.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_6.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_7.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_8.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_9.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_10.png}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth,height=0.6\textheight,keepaspectratio]{mermaid_workflows_11.png}
\end{figure}
...
accessible as of February 2026.
