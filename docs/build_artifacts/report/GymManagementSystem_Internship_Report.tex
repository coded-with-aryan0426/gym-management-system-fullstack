...
\noindent\textbf{Purpose:} Handle the creation, upgrade, and lifecycle tracking of gym memberships.

\smallskip
\noindent\textbf{How it works:}
\begin{enumerate}
  \item Member selects a membership package and submits a request.
  \item System checks whether the member already has an active membership at that gym.
  \item \textbf{If active membership exists:} Remaining days are credited back proportionally and the new package is applied with an extended end date.
  \item \textbf{If no active membership:} A new membership record is created with status \texttt{PENDING}.
  \item Start date is set to today; end date is calculated from the package duration.
  \item If the package includes PT sessions, session credits are added to the member's account.
  \item Gym owner receives a notification to approve or reject the request.
  \item On approval, status changes to \texttt{ACTIVE}.
\end{enumerate}

\noindent\textbf{Used in:} Member dashboard --- Membership Purchase and Renewal flow.
