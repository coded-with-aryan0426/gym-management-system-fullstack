...
import { subscriptionApi, licenseApi, type PlanConfig, type Subscription, type License } from '../services/subscriptionApi';
...
export function useSubscription(): UseSubscriptionResult {
  ...
  const fetchSubscription = useCallback(async () => {
    ...
    try {
      const [subResult, licenseResult, plansResult] = await Promise.all([
        subscriptionApi.getStatus(userId).catch(() => null),
        licenseApi.getMyLicense(userId).catch(() => null),
        subscriptionApi.getAllPlans().catch(() => null),
      ]);

      const subData = subResult?.data || null;
      const licenseData = licenseResult?.data || null;
      const plansData = plansResult?.data || [];

      if (subData) {
        setSubscription(subData);

        if (subData.planId && Array.isArray(plansData) && plansData.length > 0) {
          const plan = plansData.find((p: PlanConfig) => p.id === subData.planId);
          if (plan) {
            setCurrentPlan(plan);
          }
        }
  ...
  const isInGracePeriod = subscription?.isInGracePeriod === true;
  ...
  const startTrial = useCallback(async (planId: string) => {
    ...
    setIsLoading(true);
    try {
      await subscriptionApi.startTrial(userId, planId);
  ...
  const checkout = useCallback(async (planId: string, billingCycle: string) => {
    ...
    setIsLoading(true);
    try {
      const result = await subscriptionApi.checkout(userId, planId, billingCycle);
  ...
  const upgrade = useCallback(async (planId: string) => {
    ...
    setIsLoading(true);
    try {
      await subscriptionApi.upgrade(userId, planId);
      await fetchSubscription();
    } finally {
      setIsLoading(false);
    }
  ...
  const downgrade = useCallback(async (planId: string) => {
    ...
    setIsLoading(true);
    try {
      await subscriptionApi.downgrade(userId, planId);
      await fetchSubscription();
    } finally {
      setIsLoading(false);
    }
  ...
  const cancel = useCallback(async (immediate: boolean = false) => {
    ...
    setIsLoading(true);
    try {
      await subscriptionApi.cancel(userId, immediate);
      await fetchSubscription();
    } finally {
      setIsLoading(false);
    }
  ...
  const reactivate = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      await subscriptionApi.reactivate(userId);
      await fetchSubscription();
    } finally {
      setIsLoading(false);
    }
