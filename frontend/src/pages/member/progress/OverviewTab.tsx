...
import type { ProgressSummaryDTO } from '../../../services/api';
...
const OverviewTab: React.FC = () => {
    ...
    const muscleChange = summary.muscleMassChange || 0;
    const bodyFatChange = summary.bodyFatChange || 0;

    const bmiValue = summary.currentBmi?.toFixed(1) || null;
    ...
    return (
        <motion.div className="overview-tab" variants={{
            ...
            {/* Summary Banner */}
            {(weightChange !== 0 || muscleChange !== 0 || (summary.currentStreak || 0) > 0) && (
                        ...
                        <div className="summary-banner__main">
                            ...
                            <span>
                                {weightChange !== 0 || muscleChange !== 0 ? (
                                    ...
                                    <><strong>Keep going!</strong> {(summary.currentStreak || 0) > 0 ? `You're on a ${summary.currentStreak} day streak!` : 'Start logging your progress today.'}</>
                                )}
                            </span >
                        </div >
                        <div className="summary-banner__stats">
                            <div className="mini-stat">
                                <span className="mini-stat__value">{summary.currentStreak || 0}</span>
                                <span className="mini-stat__label">Day Streak</span>
                            </div>
                            <div className="mini-stat">
                                <span className="mini-stat__value">{summary.workoutsThisWeek || 0}/7</span>
            ...
            <motion.div className="stats-row" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                ...
                <div className="stat-card stat-card--bodyfat clickable">
                    ...
                    <div className="stat-card__body">
                        <div className="stat-card__value">{summary.currentBodyFat ?? '--'}<span>%</span></div>
                    ...
                    <div className="stat-card__right">
                        ...
                        {summary.currentBodyFat && (
                            <div className="stat-card__category">
                                {summary.currentBodyFat < 15 ? 'Athletic' : summary.currentBodyFat < 20 ? 'Fit' : summary.currentBodyFat < 25 ? 'Average' : 'Above Average'}
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="stat-card stat-card--muscle clickable">
                    ...
                    <div className="stat-card__body">
                        <div className="stat-card__value">{summary.currentMuscleMass ?? '--'}<span>kg</span></div>
                        <div className="stat-card__label">Muscle Mass</div>
                        <div className="stat-card__detail">
                            {summary.currentMuscleMass && summary.currentWeight ? `${((summary.currentMuscleMass / summary.currentWeight) * 100).toFixed(0)}% of total body weight` : 'Log your first measurement'}
                        </div>
                    </div>
                    <div className="stat-card__right">
                        {muscleChange !== 0 && (
