...
import type { ProgressMetricDTO, BodyMeasurementDTO } from '../../../services/api';

const MetricsTab: React.FC<{ timeRange: string }> = ({ timeRange }) => {
    const { user } = useAuth();
    const memberId = Number(user?.userId || user?.id);

    const [metrics, setMetrics] = useState<ProgressMetricDTO[]>([]);
            ...
            <div className="metrics-grid">
                                ...
                                <div className="chart-line">
                                    {metrics.slice(-10).map((metric, index) => (
                                        <div
                                            ...
                                            style={{ height: `${((metric.weight ?? 0) / Math.max(...metrics.map(m => m.weight ?? 0))) * 100}% ` }}
                                        />
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="chart-line">
                                    {metrics.slice(-10).map((metric, index) => (
                                        <div
                                            key={index}
                                            className="chart-point"
                                            style={{ height: `${((metric.bodyFat ?? 0) / Math.max(...metrics.map(m => m.bodyFat ?? 0))) * 100}% ` }}
                                        />
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="chart-line">
                                    {metrics.slice(-10).map((metric, index) => (
                                        <div
                                            key={index}
                                            className="chart-point"
                                            style={{ height: `${((metric.muscleMass ?? 0) / Math.max(...metrics.map(m => m.muscleMass ?? 0))) * 100}% ` }}
                                        />
                                    ))}
                                </div>
                            </div>
                ...
                <div className="metric-card metric-card--wide">
                    <div className="metric-card__header">
                        ...
                        <div className="metric-card__actions">
                            <span className="metric-value">Last updated: {measurements.length > 0 ? (measurements[measurements.length - 1].recordDate ? new Date(measurements[measurements.length - 1].recordDate!).toLocaleDateString() : 'Unknown') : 'Never'}</span>
                        </div>
                    </div>
