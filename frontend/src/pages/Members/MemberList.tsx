...
import { FiFilter, FiSearch, FiUserPlus, FiRefreshCw, FiPackage, FiMessageSquare, FiX, FiUsers, FiAlertTriangle, FiUserCheck, FiUserX, FiPercent, FiTrash2 } from "react-icons/fi"
...
const MemberList: React.FC = () => {
  ...
  const getExpiryInfo = (member: MemberDTO) => {
    // Timestamp-first: use endDateTime for sub-day precision
    const endDateTimeStr = (member as any).endDateTime as string | undefined
    if (endDateTimeStr) {
      const expiryDate = new Date(endDateTimeStr)
      const now = new Date()
      const msLeft = expiryDate.getTime() - now.getTime()
      const daysLeft = Math.ceil(msLeft / 86400000)
      return { date: expiryDate, daysLeft, msLeft, isExpired: msLeft < 0 }
    }
    // Fallback to date-only endDate
    const endDate = member.endDate ? new Date(member.endDate) : null
    if (!endDate) {
      const startDate = member.startDate ? new Date(member.startDate) : null
      if (!startDate || !member.planDuration) return { date: null, daysLeft: null, msLeft: null, isExpired: false }
      ...
      const daysLeft = Math.ceil(msLeft / 86400000)
      return { date: expiryDate, daysLeft, msLeft, isExpired: daysLeft < 0 }
    }
    const now = new Date(); now.setHours(0, 0, 0, 0)
    const target = new Date(endDate); target.setHours(0, 0, 0, 0)
    const daysLeft = Math.ceil((target.getTime() - now.getTime()) / 86400000)
    return { date: endDate, daysLeft, msLeft: null, isExpired: daysLeft < 0 }
  }

  const formatTimeLeft = (daysLeft: number | null, msLeft: number | null, isExpired: boolean): string => {
    if (isExpired) return `${Math.abs(daysLeft || 0)}d overdue`
    if (msLeft !== null && msLeft < 48 * 3600 * 1000 && msLeft > 0) {
  ...
  const columns: Column<MemberDTO>[] = [
    ...
    },
    {
      ...
      width: "130px",
      render: (member) => {
        const { date, daysLeft, msLeft, isExpired } = getExpiryInfo(member)
        ...
        return (
          <div className="member-expiry-cell">
            <span className={`member-days-left ${isExpired ? 'member-days-left--expired' : daysLeft !== null && daysLeft <= 7 ? 'member-days-left--warning' : ''}`}>
              {formatTimeLeft(daysLeft, msLeft, isExpired)}
            </span>
      ...
      <header className="pg-header pg-header--single-line">
        ...
        <div className="pg-stats">
          ...
          <button className={`pg-stat-card ${activeStatusFilter === 'inactive' ? 'pg-stat-card--active' : ''}`} onClick={() => setActiveStatusFilter('inactive')}>
            <div className="pg-stat-card__icon pg-stat-card__icon--expired"><FiUserX size={13} /></div>
            <div className="pg-stat-card__data">
