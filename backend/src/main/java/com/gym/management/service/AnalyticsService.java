...
public class AnalyticsService {
        ...
        public PTRevenueAnalytics getPTRevenueAnalytics(String period) {
                ...
                BigDecimal totalPTRevenue = transactions.stream()
                                .filter(t -> "PERSONAL_TRAINING".equals(t.getCategory())
                                                || "PT_SESSION".equals(t.getCategory()))
                        ...
                        User topTrainer = trainers.stream()
                                        .max((a, b) -> {
                                                long sessionsA = sessions.stream()
                                                                .filter(s -> s.getTrainer().getUserId()
                                                                                .equals(a.getUserId()))
                                                                .count();
        ...
        public TrafficHeatmapData getTrafficHeatmap(String period) {
                ...
                int[][] baseTraffic = {
                                { 5, 3, 2, 2, 8, 25, 35, 45, 30, 15, 12, 18, 35, 40, 38, 42, 55, 65, 70, 45, 25, 15, 8,
                                                4 },
                                { 4, 2, 2, 2, 7, 22, 32, 42, 28, 14, 10, 16, 32, 38, 36, 40, 52, 62, 68, 42, 22, 12, 6,
                                                3 },
                                { 6, 4, 2, 3, 9, 28, 38, 48, 32, 18, 14, 20, 38, 44, 42, 45, 58, 68, 72, 48, 28, 18, 10,
                                                5 },
                                { 5, 3, 2, 2, 8, 24, 34, 44, 30, 16, 12, 18, 34, 40, 38, 42, 54, 64, 70, 44, 24, 14, 8,
                                                4 },
                                { 7, 5, 3, 3, 10, 30, 40, 50, 35, 20, 16, 22, 40, 46, 44, 48, 60, 72, 75, 50, 30, 20,
                                                12, 6 },
                                { 12, 8, 5, 4, 6, 15, 35, 55, 65, 70, 68, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 8,
                                                6 },
                                { 10, 6, 4, 3, 5, 12, 25, 40, 50, 55, 52, 48, 45, 40, 35, 30, 25, 22, 18, 15, 12, 8, 6,
                                                5 }
                };
        ...
        public List<TrainerPerformanceInsight> getTrainerPerformance(String period) {
                ...
                return trainers.stream()
                                .map(trainer -> {
                                        ...
                                        double retentionRate = sessionCount > 0 ? Math.min(95, 75
                                                        + (completedSessions * 100.0 / Math.max(1, sessionCount)) * 0.2)
                                                        : 0;

                                        String impact = sessionCount > 40 ? "high"
                                                        : sessionCount > 20 ? "medium" : "growing";
        ...
        private List<MemberRevenueInsight> calculateTopSpenders(List<User> members, List<Transaction> transactions,
                        List<PTSession> sessions) {
                Map<Long, BigDecimal> memberSpending = new HashMap<>();
                Map<Long, Integer> memberSessions = new HashMap<>();

                for (Transaction t : transactions) {
                        if (t.getUserId() != null && t.getAmount() != null
                ...
                return members.stream()
                                ...
                                .map(m -> {
                                        ...
                                        double suppProb = Math.min(0.95,
                                                        0.3 + (spent.doubleValue() / 50000) + (sessionsCount * 0.05));
                                        ...
                                        return MemberRevenueInsight.builder()
                                                        ...
                                                        .renewalLikelihood(spent.compareTo(BigDecimal.valueOf(5000)) > 0
                                                                        ? "High"
                                                                        : "Medium")
                                                        .memberSince(m.getCreatedAt() != null
                                                                        ? m.getCreatedAt().toLocalDate()
                                                                        : LocalDate.now())
                                                        .lastVisit(LocalDate.now()
                                                                        .minusDays((long) (Math.random() * 7)))
                                                        .build();
                                })
        ...
        private StaffAttendanceRecord calculateStaffAttendance(User staff, LocalDate startOfMonth,
                ...
                for (LocalDate date = startOfMonth; !date.isAfter(endOfMonth)
        ...
        private List<AttendanceDay> generateMonthlyCalendar(LocalDate startOfMonth, LocalDate endOfMonth,
                        List<User> staff) {
