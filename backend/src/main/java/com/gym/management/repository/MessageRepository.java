...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    // Get the single most recent message in a conversation (for lastMessage
    // preview)
    java.util.Optional<Message> findTopByConversationConversationIdOrderByCreatedAtDesc(Long conversationId);

    // Count messages after the user's lastReadMessageId in this conversation (true
    // unread count)
    // Falls back to counting all non-self messages if lastReadMessageId is null
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(m) FROM Message m WHERE m.conversation.conversationId = :convId "
            +
    ...
    @org.springframework.data.jpa.repository.Query("SELECT m FROM Message m WHERE m.conversation.conversationId = :convId "
            +
            "AND m.isDeleted = false AND LOWER(CAST(m.content AS string)) LIKE LOWER(CONCAT('%', :query, '%')) " +
